(function () {
    ...
    module.controller('playlistController', function (spotifyService, spotifyFactory, nowPlayingService, Auth, $scope, $log) {
        ...
        $scope.playlistPress = function(playlist, position) {
            console.log('Playlist ' + playlist.name  + ' pressed');

            var element = angular.element(document.querySelector("#playlist_" + playlist.id));

            element.addClass('removed-playlist').one('webkitAnimationEnd oanimationend msAnimationEnd animationend', function(e) {
                ...
                $scope.results.splice(position, 1);
        ...
        $scope.trackSwipeRight = function(track, position) {
            ...
            $scope.$apply(); 
            // Applies the changes made to the scope
